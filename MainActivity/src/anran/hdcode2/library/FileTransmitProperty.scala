...
class FileTransmitProperty(dlProp:DataLinkProperty, filetype:FileType, fileLength:Int, windowId:Int=0) {
  ...
  lazy val GetHeaderLength=6//dlProp.GetBlockDataBytesCount//(8+24+1+7)/8
  ...
  val blockMap=(()=>{
    ...
    for(i<-1 until dlProp.PhysicalProperty.BlockAvailableCount){
      if(dlProp.PhysicalProperty.CenterBlockId==i)result(i)=result(i-1)
      else{
        var isparity=false
        for(p<-dlProp.InterBlockParity)
          if(p(p.length-1)==i){
            result(i)=result(i-1)
            isparity=true
          }
