...
import anran.hdcode2.util.SystemUiHider;
 ...
 * @see SystemUiHider
 */
public class ReceiverActivity extends Activity implements SensorEventListener {
  /**
  ...
  private CameraView contentView=null;
  @Override
  protected void onCreate(Bundle savedInstanceState) {
    ...
    contentView = (CameraView)findViewById(R.id.cameraView);
  ...
  View.OnTouchListener mDelayHideTouchListener = new View.OnTouchListener() {
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
      contentView.Start((FrameLayout)findViewById(R.id.receiver_layout));
  ...
  private void delayedHide(int delayMillis) {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		float magnitude = (float) (Math.sqrt(Math.pow(rawAcceleration[0], 2)
              + Math.pow(rawAcceleration[1], 2)
              + Math.pow(rawAcceleration[2], 2)) /*/ SensorManager.GRAVITY_EARTH*/);
...
//				/ SensorManager.GRAVITY_EARTH;
		...
		xyz[0] = event.values[0] - acceleration[0];
		xyz[1] = event.values[1] - acceleration[1];
		xyz[2] = event.values[2] - acceleration[2];
		contentView.setXYZMag(xyz, magnitude);
	}
}
